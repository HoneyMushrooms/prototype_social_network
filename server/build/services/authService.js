...
export default new class AuthService {
    ...
    async forgotPassword(email) {
        ...
        const { rows: [link] } = await postgresDB.query(`SELECT reset_link FROM public.reset_password WHERE user_id = $1`, [userInfo.user_id]);
        const template = MailTemplate.resetHTML(`${env.CLIENT_URL}/reset-password/${link.reset_link}`);
        await MailService.sendActivationMail(email, template);
    }
    async resetPassword(link, password) {
        const resetData = await postgresDB.query(`SELECT * FROM public.reset_password WHERE reset_link = $1`, [link]);
